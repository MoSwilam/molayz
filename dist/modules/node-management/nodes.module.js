"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.NodesModule = void 0;
const common_1 = require("@nestjs/common");
const nodes_service_1 = require("./nodes.service");
const nodes_controller_1 = require("./nodes.controller");
const mongoose_1 = require("@nestjs/mongoose");
const nodes_schema_1 = require("./nodes.schema");
const cli_lib_module_1 = require("../cli-lib/cli-lib.module");
let NodesModule = class NodesModule {
};
NodesModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forFeature([{ name: nodes_schema_1.NodeEntity.name, schema: nodes_schema_1.NodesSchema }]),
            cli_lib_module_1.CliLibModule
        ],
        controllers: [nodes_controller_1.NodesController],
        providers: [nodes_service_1.NodeService],
        exports: [nodes_service_1.NodeService]
    })
], NodesModule);
exports.NodesModule = NodesModule;
//# sourceMappingURL=nodes.module.js.map